#!/usr/bin/env python3
"""Static and package validation for the DrAnmar seal-and-divide release."""
...
CATALOG = Path("Props/SurgicalDivision/AdaptiveSealDivideRobot")
EXPECTED_PHASES = [
    "inspect", "center", "compress", "seal", "verify_seal",
    "retract_guard", "divide", "release", "verify_stumps", "complete", "abort",
]
PRIMARY_USDA_COUNT = 7
PRIMARY_GLB_COUNT = 18
PRIMARY_TEXTURE_COUNT = 7
...
def validate(
    root: Path, *, installed: bool, require_usdchecker: bool, skip_archives: bool
) -> dict[str, object]:
    root = root.resolve()
    asset_root = (
        root / "source/extensions/orbit.surgical.assets/data" / CATALOG
        if installed else root / "assets" / CATALOG
    )
    mirror_root = (
        None if installed
    ...
    counts = (len(usda), len(glb), len(textures))
    expected = (PRIMARY_USDA_COUNT, PRIMARY_GLB_COUNT, PRIMARY_TEXTURE_COUNT)
    if counts != expected:
        raise AssertionError(f"Catalog counts {counts} do not match {expected}")
    for path in glb:
        assert_glb(path)
    ...
    if installed:
        json_paths = list(asset_root.glob("*.json")) + [
            root / "physics_next/surgical-division/dranmar-adaptive-seal-divide-v1.json",
    ...
    task = json.loads(
        (asset_root / "adaptive_seal_divide_task_contract.json").read_text()
    )
    if task.get("phases") != EXPECTED_PHASES:
        raise AssertionError("Task phases do not match the canonical sequence")
    profile = json.loads((asset_root / "physics_profile.json").read_text())
    if profile["tool"]["joint_count"] != 8:
        raise AssertionError("Mechanism must expose exactly 8 controlled joints")
    if profile["tool"]["bridge_pin_count"] != 8:
    ...
    if not installed:
        ...
        primary = sorted(
            path.relative_to(asset_root) for path in asset_root.rglob("*")
            if path.is_file()
        )
        mirror = sorted(
            path.relative_to(mirror_root) for path in mirror_root.rglob("*")
            if path.is_file()
        )
        if primary != mirror:
    ...
    if not installed and not skip_archives:
        parent = root.parent
        for archive in (
            parent / "dranmar_adaptive_seal_divide_robot_v0.1.0.zip",
            parent / "dranmar_adaptive_seal_divide_robot_catalog_v0.1.0.zip",
            parent / "dranmar_adaptive_seal_divide_robot_repo_overlay_v0.1.0.zip",
        ):
...
def main() -> None:
    ...
    print(json.dumps(validate(
        args.root, installed=args.installed,
