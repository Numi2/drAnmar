...
class ExpertDemonstrationController:
    ...
    def _procedure_instruction(self) -> str | None:
        ...
        return "The physical ring pass and needle handoff are complete. Inspect the strand before taking control."
    ...
    def _follow_needle_through_ring(
        self,
        action: np.ndarray,
        tools: dict[int, np.ndarray],
        arm: int,
        needle_position: np.ndarray | None,
    ) -> bool:
        """Drive the physically grasped needle through the authored ring path."""
        if needle_position is None or not len(self.waypoints):
        ...
        tracked_needle = np.asarray(needle_position, dtype=np.float32)
    ...
    def _manipulate(
        ...
        if kind == "softmimicgen_threading":
            ...
            if self.manipulation_step == 0:
                grippers[primary_arm] = False
                path_complete = self._follow_needle_through_ring(
                    action,
                    tools,
                    primary_arm,
                    object_position,
            ...
            if not self.ring_handoff_complete:
                if not self._threaded_ring_handoff(
                    ...
                    return False
            return self.ring_handoff_complete
        if len(self.waypoints):
            rotation = (0.024, 0.0, 0.0) if kind in {"needle_pass", "softmimicgen_threading"} else (0.0, 0.0, 0.0)
    ...
    def step(
        ...
        if self.guide_kind == "softmimicgen_threading" and self.primary_arm is None and object_position is not None:
            candidates = [
        ...
        phase_changed = False
        completed = False
        primary_arm = self.primary_arm if self.guide_kind == "softmimicgen_threading" and self.primary_arm is not None else 0
