...
function releasePageResources(){if(pageDisposed)return;pageDisposed=true;queuedDrive=null;queuedBimanual=null;clearInterval(driveInterval);clearInterval(refreshInterval);clearInterval(heartbeatInterval);if(gamepadAnimationFrame!==null)cancelAnimationFrame(gamepadAnimationFrame);if(macroPulseTimer)clearTimeout(macroPulseTimer);if(voicePulseTimer)clearTimeout(voicePulseTimer);if(keyFlashTimer)clearTimeout(keyFlashTimer);if(toastTimer)clearTimeout(toastTimer);if(cameraAdjustTimer)clearTimeout(cameraAdjustTimer);try{voiceRecognition?.abort()}catch(_error){}voiceRecognition=null;voiceListening=false;gamepadButtonStates.clear();gamepadKnownIndices.clear();latestGamepadCommands=new Map();activeFetchControllers.forEach(controller=>controller.abort());activeFetchControllers.clear();clearHeldControls();const image=document.getElementById('cameraImage');image.removeAttribute('src');const options={method:'POST',headers:{'content-type':'application/json','x-dr-anmar-operator':operatorId},body:JSON.stringify({source:'keyboard_pointer'}),keepalive:true};fetch('/api/stop',options).catch(()=>{});fetch('/api/operator/release',{...options,body:'{}'}).catch(()=>{})}
auditKeyboardCoverage();if(!(window.SpeechRecognition||window.webkitSpeechRecognition))setVoiceStatus('Microphone unavailable here · type commands instead');updateGamepadStatus();const driveInterval=setInterval(updateDrive,33),refreshInterval=setInterval(refresh,500),heartbeatInterval=setInterval(heartbeat,10000);gamepadAnimationFrame=requestAnimationFrame(pollGamepads);window.addEventListener('pagehide',releasePageResources,{once:true});window.addEventListener('pageshow',event=>{if(event.persisted&&pageDisposed)location.reload()});document.addEventListener('visibilitychange',()=>{if(!document.hidden){refresh();heartbeat()}});refresh();
</script></body></html>"""
