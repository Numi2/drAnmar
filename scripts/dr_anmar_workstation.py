...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.35 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI";min-height:100vh}
    ...
    main{display:grid;grid-template-columns:minmax(0,1fr) 380px;height:calc(100vh - 58px)}
    .view{position:relative;overflow:hidden;background:#020608;display:flex;align-items:center;justify-content:center}.view img{width:100%;height:100%;object-fit:contain}.view-toolbar{margin:0 0 12px;padding:9px;border:1px solid #294956;border-radius:10px;background:#07151c}.view-toolbar-row{display:grid;grid-template-columns:62px 1fr;align-items:center;gap:7px}.view-toolbar-row+.view-toolbar-row{margin-top:7px}.view-toolbar-label{color:#6f909b;font:800 9px/1 ui-monospace,SFMono-Regular,Menlo;letter-spacing:.1em;text-transform:uppercase}.camera-tabs,.view-presets{display:grid;gap:5px}.camera-tabs{grid-template-columns:repeat(4,1fr)}.view-presets{grid-template-columns:repeat(3,1fr)}.camera-tabs button,.view-presets button{min-height:32px;padding:0 5px;font-size:9px}.camera-tabs button.active,.view-presets button.active{background:var(--cyan);border-color:var(--cyan);color:#031014}.gaze-cursor{position:absolute;width:18px;height:18px;border:1px solid #fff;border-radius:50%;translate:-50% -50%;pointer-events:none;opacity:0;box-shadow:0 0 0 3px #2cd2e855}.view.gaze-on .gaze-cursor{opacity:.85}
    ...
    @media(max-width:1100px){main{grid-template-columns:minmax(0,1fr) 350px}.shortcut-columns{grid-template-columns:repeat(2,1fr)}.shortcut-group.wide{grid-column:span 2}.shortcut-group.wide .shortcut-list{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:880px){main{display:block;height:auto}.view{height:52vh}aside{border-left:0;border-top:1px solid var(--line)}header{padding:0 12px}.tag{display:none}.view-toolbar-row{grid-template-columns:52px 1fr}}
  </style>
</head>
<body>
...
<main>
  ...
  <aside>
    <section class="view-toolbar" aria-label="Camera controls"><div class="view-toolbar-row"><span class="view-toolbar-label">Camera</span><div class="camera-tabs"><button class="active" data-camera="endoscope_left" data-shortcut="3" onclick="setCamera('endoscope_left',this)">Left <kbd>3</kbd></button><button data-camera="endoscope_right" data-shortcut="4" onclick="setCamera('endoscope_right',this)">Right <kbd>4</kbd></button><button data-camera="wrist_1" data-shortcut="5" onclick="setCamera('wrist_1',this)">Wrist 1 <kbd>5</kbd></button><button id="wrist2Tab" class="hidden" data-camera="wrist_2" data-shortcut="6" onclick="setCamera('wrist_2',this)">Wrist 2 <kbd>6</kbd></button></div></div><div class="view-toolbar-row"><span class="view-toolbar-label">Framing</span><div class="view-presets"><button class="active" data-view-mode="operative" data-shortcut="7" onclick="setCameraView('operative',this)">Operative <kbd>7</kbd></button><button data-view-mode="close" data-shortcut="8" onclick="setCameraView('close',this)">Close <kbd>8</kbd></button><button data-view-mode="overview" data-shortcut="9" onclick="setCameraView('overview',this)">Overview <kbd>9</kbd></button></div></div></section>
...
def analyze_demo(
    ...
    hoop_passes = series("hoop_pass_count", np.int32)
    hoop_contacts = series("hoop_ring_contacts", np.int32)
    hoop_center_error = series("hoop_center_error_m")
    ...
    elif procedure_id == "needle-hoop-threading" and len(hoop_passes):
        pass_score = 1.0 if np.any(hoop_passes >= 1) else 0.0
        clean_score = 1.0 if len(hoop_clean) and np.any(hoop_clean) else 0.0
    ...
    if len(hoop_passes) and np.any(hoop_passes >= 1):
        hoop_index = _first_index(hoop_passes >= 1)
        if hoop_index is not None and hoop_index < len(times):
            timeline.append({"time_s": round(float(times[hoop_index]), 2), "kind": "task", "label": "Needle crossed the hoop plane"})
    ...
    return {
        ...
        "metrics": {
            ...
            "hoop_passes": int(np.max(hoop_passes)) if len(hoop_passes) else 0,
...
def main() -> None:
    ...
    if organ_usd.is_file():
        ...
        if guide_kind in THREAD_GUIDE_KINDS:
            ...
            suture_curve.CreateWidthsAttr(Vt.FloatArray([suture_width] * suture_model.node_count))
            ...
            suture_shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(suture_color)
            suture_shader.CreateInput("emissiveColor", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(0.0, 0.0, 0.0))
            suture_shader.CreateInput("metallic", Sdf.ValueTypeNames.Float).Set(0.0)
            suture_shader.CreateInput("roughness", Sdf.ValueTypeNames.Float).Set(0.78)
        ...
        if guide_kind == "hoop_threading" and len(room_waypoints) >= 3:
            hoop_center = room_waypoints[1]
            hoop_normal = room_waypoints[2] - room_waypoints[0]
            hoop_normal /= max(float(np.linalg.norm(hoop_normal)), 1e-8)
            reference = np.asarray((0.0, 0.0, 1.0), dtype=np.float32)
            if abs(float(np.dot(reference, hoop_normal))) > 0.92:
                reference = np.asarray((0.0, 1.0, 0.0), dtype=np.float32)
            hoop_axis_u = np.cross(hoop_normal, reference)
            hoop_axis_u /= max(float(np.linalg.norm(hoop_axis_u)), 1e-8)
            hoop_axis_v = np.cross(hoop_normal, hoop_axis_u)
            inner_radius = float(procedure.get("hoop_inner_radius_m", 0.017))
            tube_radius = float(procedure.get("hoop_tube_radius_m", 0.0025))
            major_radius = inner_radius + tube_radius
            major_segments, tube_segments = 48, 12
            hoop_points = []
            for major_index in range(major_segments):
                major_angle = 2.0 * np.pi * major_index / major_segments
                radial_direction = np.cos(major_angle) * hoop_axis_u + np.sin(major_angle) * hoop_axis_v
                for tube_index in range(tube_segments):
                    tube_angle = 2.0 * np.pi * tube_index / tube_segments
                    hoop_points.append(
                        hoop_center
                        + radial_direction * (major_radius + tube_radius * np.cos(tube_angle))
                        + hoop_normal * (tube_radius * np.sin(tube_angle))
                    )
            hoop_faces = []
            for major_index in range(major_segments):
                ...
                for tube_index in range(tube_segments):
                    ...
                    hoop_faces.extend(
                        (
                            major_index * tube_segments + tube_index,
                            next_major * tube_segments + tube_index,
                            next_major * tube_segments + next_tube,
                            major_index * tube_segments + next_tube,
                        )
                    )
            hoop_mesh = UsdGeom.Mesh.Define(stage, "/World/envs/env_0/DrAnmarNeedleHoop")
            hoop_mesh.CreatePointsAttr(Vt.Vec3fArray.FromNumpy(np.asarray(hoop_points, dtype=np.float32)))
            hoop_mesh.CreateFaceVertexCountsAttr(Vt.IntArray([4] * (major_segments * tube_segments)))
            ...
            stand.CreateRadiusAttr(0.003)
            stand.CreateHeightAttr(stand_top)
            ...
            base.CreateHeightAttr(0.003)
            UsdGeom.Xformable(base.GetPrim()).AddTranslateOp().Set(
                Gf.Vec3d(float(hoop_center[0]), float(hoop_center[1]), 0.0015)
            )
    ...
    while simulation_app.is_running() and not stop_event.is_set():
        ...
        procedure_needle_points = (
            needle_tip_positions_world() if needle_interaction_enabled else np.empty((0, 3), dtype=np.float32)
        )
        expert_knot_guidance = bool(
        ...
        if is_recording:
            ...
            frame = {
                ...
                "hoop_pass_count": np.array(hoop_metrics.get("pass_count", 0), dtype=np.int16),
                "hoop_ring_contacts": np.array(hoop_metrics.get("ring_contacts", 0), dtype=np.int16),
                "hoop_center_error_m": np.array(hoop_metrics.get("center_error_m", np.nan), dtype=np.float32),
