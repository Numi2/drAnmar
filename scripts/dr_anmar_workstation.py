...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.35 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI";min-height:100vh}
    ...
    .teleop-strip{grid-column:1/-1;display:grid;grid-template-columns:.72fr 1.28fr;gap:5px}.gamepad-status{min-width:0;min-height:34px;padding:4px 7px;text-align:left;border-color:#294b57;background:#081820;color:#a9c2ca;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.gamepad-status.connected{border-color:#387c68;color:var(--green);background:#0a241f}.voice-form{display:grid;grid-template-columns:minmax(0,1fr) 36px 36px;gap:4px}.voice-form input{min-width:0;height:34px;padding:0 8px;border:1px solid #294b57;border-radius:6px;background:#061219;color:#ddf7fa;font:9px/1 ui-sans-serif,system-ui}.voice-form input:focus{outline:1px solid var(--cyan);border-color:var(--cyan)}.voice-form button{min-height:34px;padding:0;font-size:12px}.voice-mic.listening{border-color:#ff8b93;background:#4b1f28;color:#fff;box-shadow:0 0 12px #ff4f6670}.voice-status{grid-column:1/-1;min-height:12px;color:#718f99;font:8px/1.2 ui-monospace,SFMono-Regular,Menlo;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.voice-status.listening{color:#ff9da5}.voice-status.ok{color:var(--green)}.voice-status.error{color:#ffb1b6}
...
<div id="keyboardHelp" class="keyboard-help hidden" role="dialog" aria-modal="true" aria-labelledby="keyboardHelpTitle"><div class="keyboard-help-panel"><div class="keyboard-help-head"><div><h1 id="keyboardHelpTitle">Two-hand surgical game controls</h1><p>Each hand permanently owns one robot. Hold keys to move both at once; releasing stops that motion. Backspace or Escape stops everything.</p></div><button data-shortcut="?" onclick="toggleKeyboardHelp(false)">Close <kbd>?</kbd></button></div><div class="shortcut-columns">
  ...
  <div class="shortcut-group"><h3>XBOX CONTROLLER</h3><div class="shortcut-line"><kbd>Left stick</kbd><span>Toward / away + left / right</span></div><div class="shortcut-line"><kbd>Right stick</kbd><span>Pitch + yaw wrist</span></div><div class="shortcut-line"><kbd>LB / RB</kbd><span>Roll wrist</span></div><div class="shortcut-line"><kbd>LT / RT</kbd><span>Down / up</span></div><div class="shortcut-line"><kbd>A / B</kbd><span>Grip / emergency stop</span></div><div class="shortcut-line"><kbd>X / Y</kbd><span>Smart assist / next camera</span></div><div class="shortcut-line"><kbd>D-pad</kbd><span>Select robot + fine / fast</span></div><div class="shortcut-line"><kbd>View / Menu</kbd><span>Sensor / free camera</span></div></div>
...
function gamepadPressedOnce(pad,buttonIndex){const key=`${pad.index}:${buttonIndex}`,pressed=!!pad.buttons[buttonIndex]?.pressed,previous=gamepadButtonStates.get(key)||false;gamepadButtonStates.set(key,pressed);return pressed&&!previous}
function updateGamepadStatus(pads=[]){const button=document.getElementById('gamepadStatus');if(!button)return;const arms=latestStatus?.arms||1,label=!pads.length?'🎮 Connect Xbox controller':pads.length>1&&arms>1?'🎮 2 controllers · both robots':`🎮 Xbox · Instrument ${activeArm+1}`;if(button.textContent!==label)button.textContent=label;button.classList.toggle('connected',pads.length>0)}
function readGamepadCommands(){const pads=(navigator.getGamepads?[...navigator.getGamepads()]:[]).filter(Boolean),commands=new Map(),arms=Math.min(latestStatus?.arms||1,2),livePrefixes=new Set(pads.map(pad=>`${pad.index}:`));for(const key of gamepadButtonStates.keys())if(![...livePrefixes].some(prefix=>key.startsWith(prefix)))gamepadButtonStates.delete(key);updateGamepadStatus(pads);if(!pads.length){gamepadSafetyLatched=false;return commands}const assigned=pads.slice(0,Math.min(pads.length,arms));let anyMotion=false;assigned.forEach((pad,ordinal)=>{if(pads.length===1){if(gamepadPressedOnce(pad,14))setArm(0);if(gamepadPressedOnce(pad,15)&&arms>1)setArm(1)}const arm=pads.length>1?ordinal:Math.min(activeArm,arms-1),values=Array(6).fill(0);values[1]-=deadzone(pad.axes[0]||0);values[0]+=deadzone(pad.axes[1]||0);values[5]+=deadzone(pad.axes[2]||0);values[4]+=deadzone(pad.axes[3]||0);values[3]+=(pad.buttons[5]?.value||0)-(pad.buttons[4]?.value||0);values[2]+=(pad.buttons[7]?.value||0)-(pad.buttons[6]?.value||0);if(gamepadPressedOnce(pad,0))toggleGrip(arm,'gamepad');if(gamepadPressedOnce(pad,1)){gamepadSafetyLatched=true;emergencyStop('gamepad')}if(gamepadPressedOnce(pad,2))smartAction('gamepad_smart_action');if(gamepadPressedOnce(pad,3))cycleCameraView();if(gamepadPressedOnce(pad,8))cycleSensorCamera();if(gamepadPressedOnce(pad,9))toggleFreeCamera();if(gamepadPressedOnce(pad,12))setHandSpeed(arm,1.7,arm===0?'3':'0');if(gamepadPressedOnce(pad,13))setHandSpeed(arm,.35,arm===0?'1':'8');const normalized=normalizeDrive(values);anyMotion=anyMotion||normalized.some(value=>Math.abs(value)>.01);commands.set(arm,{values:normalized,labels:['Xbox controller']})});if(gamepadSafetyLatched){const stopHeld=pads.some(pad=>pad.buttons[1]?.pressed);if(!anyMotion&&!stopHeld)gamepadSafetyLatched=false;else commands.clear()}return commands}
...
function buildBimanualCommands(gamepadCommands=new Map()){const arms=Math.min(latestStatus?.arms||1,2),commands=[];for(let arm=0;arm<arms;arm++){const hand=keyboardArmDrive(arm),pad=gamepadCommands.get(arm),values=hand.values.slice(),labels=hand.labels.slice();if(pad){pad.values.forEach((value,index)=>values[index]+=value);labels.push(...pad.labels)}commands.push({arm,values:normalizeDrive(values),speed:keyboardSpeeds[arm],labels,rotate:hand.rotate})}return commands}
...
window.addEventListener('gamepadconnected',event=>{updateGamepadStatus((navigator.getGamepads?[...navigator.getGamepads()]:[]).filter(Boolean));toast(`${event.gamepad.id||'Controller'} connected`)});window.addEventListener('gamepaddisconnected',()=>{gamepadButtonStates.clear();updateGamepadStatus((navigator.getGamepads?[...navigator.getGamepads()]:[]).filter(Boolean));if(!pageDisposed)emergencyStop('gamepad')});
...
function releasePageResources(){if(pageDisposed)return;pageDisposed=true;queuedDrive=null;queuedBimanual=null;clearInterval(driveInterval);clearInterval(refreshInterval);clearInterval(heartbeatInterval);if(macroPulseTimer)clearTimeout(macroPulseTimer);if(voicePulseTimer)clearTimeout(voicePulseTimer);if(keyFlashTimer)clearTimeout(keyFlashTimer);if(toastTimer)clearTimeout(toastTimer);if(cameraAdjustTimer)clearTimeout(cameraAdjustTimer);try{voiceRecognition?.abort()}catch(_error){}voiceRecognition=null;voiceListening=false;gamepadButtonStates.clear();activeFetchControllers.forEach(controller=>controller.abort());activeFetchControllers.clear();clearHeldControls();const image=document.getElementById('cameraImage');image.removeAttribute('src');const options={method:'POST',headers:{'content-type':'application/json','x-dr-anmar-operator':operatorId},body:JSON.stringify({source:'keyboard_pointer'}),keepalive:true};fetch('/api/stop',options).catch(()=>{});fetch('/api/operator/release',{...options,body:'{}'}).catch(()=>{})}
auditKeyboardCoverage();if(!(window.SpeechRecognition||window.webkitSpeechRecognition))setVoiceStatus('Microphone unavailable here · type commands instead');updateGamepadStatus();const driveInterval=setInterval(updateDrive,50),refreshInterval=setInterval(refresh,500),heartbeatInterval=setInterval(heartbeat,10000);window.addEventListener('pagehide',releasePageResources,{once:true});window.addEventListener('pageshow',event=>{if(event.persisted&&pageDisposed)location.reload()});document.addEventListener('visibilitychange',()=>{if(!document.hidden){refresh();heartbeat()}});refresh();
</script></body></html>"""
