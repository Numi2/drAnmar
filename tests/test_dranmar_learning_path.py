...
def test_block_lift_requires_physics_owned_height_and_sustained_contact() -> None:
    ...
    assert "((phase == 1) & giver_bilateral_contact)" in benchmark_source
    assert "| (phase == 2)" in benchmark_source
    ...
    assert (
        '"giver_lift_requires_current_bilateral_contact": True'
        in benchmark_source
    )
    assert (
        '"giver_transport_tolerates_contact_flicker": True'
        in benchmark_source
    )
