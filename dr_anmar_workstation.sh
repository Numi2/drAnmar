...
case "${1:-status}" in
    start)
        ...
        rm -f "${PID_FILE}"
        cd "${ORBIT_ROOT}"
        nohup ./dr_anmar.sh workstation "${PORT}" "${TASK}" "${PROCEDURE}" "${ANATOMY_SCENE}" "${ANATOMY_SCENE_ID}" "${ANATOMY_TITLE}" "${OPENUSD_ENVIRONMENT}" "${BENCH_ASSETS}" "${GRIPPER_OPEN_RAD}" "${GRIPPER_CLOSE_RAD}" >>"${LOG_FILE}" 2>&1 &
        echo "$!" >"${PID_FILE}"
    ...
    restart)
        "$0" stop || true
        "$0" start "${PORT}" "${TASK}" "${PROCEDURE}" "${ANATOMY_SCENE}" "${ANATOMY_SCENE_ID}" "${ANATOMY_TITLE}" "${OPENUSD_ENVIRONMENT}" "${BENCH_ASSETS}" "${GRIPPER_OPEN_RAD}" "${GRIPPER_CLOSE_RAD}"
        ;;
    restart-anatomy)
        room_id="${3:?A room id is required}"
        scene="${4:?An official main_scene.usd path is required}"
