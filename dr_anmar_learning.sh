...
case "${command}" in
    ...
    receiver-custody-controller)
        ...
        "${DR_ANMAR_ISAAC_PYTHON}" \
            ...
            "${dataset_args[@]}"
        ;;
    receiver-custody-replicate)
        require_runtime
        controller_checkpoint="${2:-}"
        risk_checkpoint="${3:-}"
        output="${4:-}"
        if [[ -z "${controller_checkpoint}" || -z "${risk_checkpoint}" || -z "${output}" || "$#" -lt 7 ]]; then
            usage
            exit 2
        fi
        shift 4
    ...
    receiver-custody-delay-gate)
        ...
        "${DR_ANMAR_ISAAC_PYTHON}" \
            ...
            "${dataset_args[@]}"
        ;;
    receiver-custody-preprobe-risk)
        require_runtime
        teacher_checkpoint="${2:-}"
        output="${3:-}"
        preprobe_checkpoint="${4:-}"
        if [[ -z "${teacher_checkpoint}" || -z "${output}" || -z "${preprobe_checkpoint}" || "$#" -lt 7 ]]; then
            usage
            exit 2
        fi
        shift 4
        ...
        "${DR_ANMAR_ISAAC_PYTHON}" \
            "${REPO_ROOT}/scripts/train_dranmar_preprobe_risk_model.py" \
            --teacher_checkpoint "${teacher_checkpoint}" \
            --output "${output}" \
            --checkpoint "${preprobe_checkpoint}" \
            "${dataset_args[@]}"
        ;;
    ...
    receiver-custody-interventions)
        ...
        exec env \
            ...
            "${6:-DrAnmar-Handover-Needle-Dual-PSM-IK-Rel-v0}"
        ;;
    receiver-custody-delay-interventions)
        dataset="${2:-}"
        if [[ -z "${dataset}" ]]; then
            usage
            exit 2
        ...
        exec env \
            DR_ANMAR_RECEIVER_ACTIVE_CUSTODY_VERIFICATION=1 \
            DR_ANMAR_RECEIVER_ACTIVE_CUSTODY_INTERVENTION=1 \
            DR_ANMAR_RECEIVER_ACTIVE_CUSTODY_INTERVENTION_PROFILE=release_delay \
            DR_ANMAR_RECEIVER_ACTIVE_CUSTODY_PROBE_DATASET="${dataset}" \
            DR_ANMAR_SEED_STREAM_OFFSET="${7:-0}" \
            "${BASH_SOURCE[0]}" promoted-handover \
            "${3:-1200}" \
            "${4:-2000}" \
            "${5:-${DR_ANMAR_LEARNING_OUTPUT}/receiver-custody-delay-interventions}" \
            "${6:-DrAnmar-Handover-Needle-Dual-PSM-IK-Rel-v0}"
        ;;
    receiver-custody-retry-interventions)
        dataset="${2:-}"
        if [[ -z "${dataset}" ]]; then
            usage
            exit 2
    ...
    play)
        ...
        if [[ -n "${DR_ANMAR_RECEIVER_ACTIVE_CUSTODY_INTERVENTION_PROFILE:-}" ]]; then
            pickup_recovery_args+=(
