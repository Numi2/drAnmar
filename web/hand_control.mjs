...
class HandController {
  ...
  bind() {
    // The launcher is overlaid inside #cameraView. Keep its pointer press from
    // reaching the camera-orbit handler, which captures the pointer and
    // retargets the eventual click away from this button.
  ...
  async start() {
    ...
    try {
      ...
      // Use a saved personal calibration when available and a bounded
      // provisional span otherwise. A stable tracked hand should reach
      // control directly instead of entering a three-stage setup wizard.
      this.calibrationActive = false;
      this.calibrationAutomatic = false;
      this.calibrationCapture = null;
