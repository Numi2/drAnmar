...
from isaaclab.utils.math import compute_pose_error, quat_apply, quat_from_matrix, quat_mul
...
class PenetrationAnalyticController(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.normalized_contact_threshold = 1.0e-6
    ...
    def forward(self, raw: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        grasp_quaternion = torch.tensor(
            ...
            device=raw.device,
            dtype=raw.dtype,
        ).expand_as(desired_needle_quaternion)
        ...
        rotation = (orientation_error / self.rotation_scale_rad).clamp(-0.2, 0.2)
        ...
        unsafe = (~bilateral) | ((phase <= 2) & (normalized_force > self.normalized_force_limit))
        retreat = torch.cat(
            (surface_normal * self.normal_advance_limit, torch.zeros_like(rotation)), dim=-1
        )
        base = torch.where(unsafe.unsqueeze(-1), retreat, base)
