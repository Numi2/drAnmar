...
def process_actions(teleop_interface, env, is_dual) -> torch.Tensor:
    ...
    if is_dual:
        ...
        if "Reach" in args_cli.task:
            actions = torch.concat([delta_pose_0, delta_pose_1], dim=1)
        else:
            gripper_0 = torch.full(
                (delta_pose_0.shape[0], 1), -1.0 if gripper_command_0 else 1.0, device=env.unwrapped.device
            )
            gripper_1 = torch.full(
                (delta_pose_1.shape[0], 1), -1.0 if gripper_command_1 else 1.0, device=env.unwrapped.device
            )
            # Dual-arm action managers expect both 6-DoF tool commands first,
            # followed by the two gripper commands.
            actions = torch.concat([delta_pose_0, delta_pose_1, gripper_0, gripper_1], dim=1)
    else:
        # get keyboard command
